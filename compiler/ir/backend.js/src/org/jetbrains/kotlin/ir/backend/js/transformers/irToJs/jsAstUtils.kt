...
private inline fun <T : JsNode> T.addSourceInfoIfNeed(node: IrElement, context: JsGenerationContext) {
    ...
    val sourceInputStream: InputStream? = try {
        FileInputStream(sourceInfo.file)
    ...
    locationTarget.source = if (sourceInputStream != null)
        JsLocationWithEmbeddedSource(sourceInfo) {
            InputStreamReader(sourceInputStream, StandardCharsets.UTF_8)
        }
    else
        sourceInfo
}
